...
if (typeof document !== "undefined") {
  ...
  function showResult(prob) {
    ...
    const base = META.base_rate || 0.15;
    const rel = prob / base;
    // bands are relative to the dataset's average student, calibrated to
    // this model's real output range (extreme ~2.6x avg; low<1.2x, high>=2.2x)
    const band = rel < 1.2 ? ["low", "Low risk"]
               : rel < 2.2 ? ["moderate", "Moderate risk"]
               : ["high", "High risk"];
    const relText = rel < 0.75 ? "below" :
      `about ${rel < 10 ? rel.toFixed(1) : Math.round(rel)}&times;`;
    const meterPct = Math.min(100, Math.round(rel * 33)); // 3x avg fills the bar
    document.getElementById("result").innerHTML = `
      <p class="slip-eyebrow">Screening result</p>
      <p class="verdict ${band[0]}">${band[1]}</p>
      <div class="meter" role="img"
           aria-label="Estimated probability ${pct} percent, ${band[1]}">
        <div class="meter-fill ${band[0]}" style="width:${meterPct}%"></div>
      </div>
      <p class="pct"><span>${pct}%</span> estimated chance of finishing below
      the passing grade (${META.passing_grade}/20) &mdash; ${relText} the
      dataset average of ${Math.round(base * 100)}%</p>
